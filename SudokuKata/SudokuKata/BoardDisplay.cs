...
public class BoardDisplay
{
    ...
    public BoardDisplay(int customBlockSize = DefaultBlockSize) : this(new int[customBlockSize * customBlockSize * customBlockSize * customBlockSize], customBlockSize)
    { }

    public BoardDisplay(string board) : this(board
        .Where(c => c == '.' || char.IsDigit(c))
        .Select(c => c switch { '.' => 0, _ => (int)(c - '0') }).ToArray(), DefaultBlockSize)
    { }

    private BoardDisplay(int[] initialState, int customBlockSize)
    {
